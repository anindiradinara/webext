...
if (in_array('woocommerce/woocommerce.php', 
    ...
    add_filter ('woocommerce_default_address_fields', 'override_default_address',1000);
	...
	function epeken_js_change_select_class() {
			...
			if($epeken_wp_version < 5.6)
			   wp_enqueue_script('init_controls',plugins_url('/js/init_controls.js',__FILE__), array('jquery'),'1.1.8.6.15');
			else
			   wp_enqueue_script('init_controls',plugins_url('/js/init_controls_wp.5.6.js',__FILE__), array('jquery'),'1.1.8.6.17');
	...
	function epeken_js_query_kota_shipping_form() {
		...
		wp_enqueue_script('ajax_shipping_kota',plugins_url('/js/shipping_kota.js',__FILE__), array('jquery'));
		wp_localize_script( 
	...
	function epeken_js_query_kota_billing_form() {
		...
		$url = admin_url('admin-ajax.php');
		wp_enqueue_script('ajax_billing_kota',plugins_url('/js/billing_kota.js',__FILE__), array('jquery'));
		wp_localize_script( 
...
function epeken_checkout_field_update_order_meta( $order_id ) {
    ...
    if ( $_SESSION['isshippedifadr'] === '1' ) { //if shipped to other address.
...
add_action ('woocommerce_single_product_summary', 'epeken_display_kota_asal_pengiriman');
 function epeken_display_kota_asal_pengiriman() {
  ...
  if(!function_exists('epeken_dequeue_jquery_migrate')) {
   ...
   add_action( 'wp_default_scripts', 'epeken_dequeue_jquery_migrate' );
  }
  ...
  function epeken_register_scripts() {
	  ...
	  $epeken_wp_ver = substr($wp_version,0,3);
  	  if($t) {
		  ...
		  wp_enqueue_style('epeken_style_checkout',plugins_url('assets/epeken-style-checkout.css',__FILE__));
