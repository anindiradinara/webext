...
function epeken_submit_konfirmasi_pembayaran() {
	...
	$is_payment_confirmed = get_post_meta($order_id,'payment_confirmation', true);
		 ...
		 if($order == false) {
			 $_SESSION['result_msg'] = "Order number tidak ditemukan. Silakan mencoba lagi.";
			 return;
		 }
		...
		$_SESSION['result_msg'] = 'Konfirmasi pembayaran sudah dilakukan untuk Pesanan #'.$order_id.'. Kami akan segera memproses pesanan Anda. Terima kasih.';
		...
		return;
	      ...
	      $_SESSION['result_msg'] = 'Wajib upload bukti pembayaran. Mohon coba kembali. Terima kasih.';
	      return;
    	}
	if (empty($order_id) || empty($tgl_pembayaran) || empty ($nama_pembayar) || empty($nama_bank) || empty($transfer_amount)) {
		...
		$_SESSION['result_msg'] = 'Semua Field Harus Diisi, Tidak boleh kosong';
		return;
	}
	if ($order == false) {
		echo "Order ".$order_id.__(" not found",'epeken-all-kurir');
		$_SESSION['result_msg'] = "Order ".$order_id." tidak ditemukan";
		return;
	}
	if($is_payment_confirmed === 'yes')
	{
		echo __("You have confirmed this order payment. We are processing this order. Thank you.", 'epeken-all-kurir');
		$_SESSION['result_msg'] = "Konfirmasi pembayaran sudah dilakukan untuk order ini. Kami akan segera memproses order ini. Terima kasih.";
		return;
	}
	if(!empty($order->post)) {
